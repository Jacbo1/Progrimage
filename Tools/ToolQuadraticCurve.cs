...
{
	internal class ToolQuadraticCurve : ITool
	{
		#region Fields
		// Public fields
		public const string CONST_NAME = "Quadratic Curve";

		// Private
		private DrawingGrabPoint[] _dragPoints = new DrawingGrabPoint[3];
		private DrawingQuadraticCurve _curve = new();
		...
		{
			_pointOverlay = new(layer, 0, _dragPoints[0], _dragPoints[1], _dragPoints[2]) { Hidden = true };
		...
		public void OnMouseMoveCanvasDouble(double2 pos)
		{
			...
			{
				_curve.Points[1] = _dragPoints[1].Pos = (pos + _dragPoints[0].Pos) * 0.5;
				_curve.Points[2] = _dragPoints[2].Pos = pos;
