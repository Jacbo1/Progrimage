...
namespace Progrimage.Utils
{
  ...
  //      public static Image<TPixel> GetSubImage<TPixel>(this Image<TPixel> src, int2 pos, int2 size) where TPixel: unmanaged, IPixel<TPixel>
  //      {
  ...
  //          var subimage = new Image<TPixel>(size.x, size.y);
		...
		//	int2 overlapMin = Math2.Max(pos, 0);
  //          int2 overlapMax = Math2.Min(pos + size, new int2(src.Width, src.Height));
  ...
  //          {
		//		var srcRow = src.DangerousGetPixelRowMemory(y + srcOffset.y).Span;
  ...
  //          return subimage;
        ...
        public static Image<Argb32> HighQualityDownscale(Image<Argb32> image, int2 size, bool scaleToFit = false)
        {
            int2 currentSize = new int2(image.Width, image.Height);
			if (size.x < 1 || size.y < 1 || size == currentSize)
				return image;
			...
			if (scaleToFit)
			{
				...
				scale = currentSize / (double2)size;
				if (scale.x > scale.y) size.y = (int)Math.Round(currentSize.y / scale.x, MidpointRounding.AwayFromZero);
				else size.x = (int)Math.Round(currentSize.x / scale.y, MidpointRounding.AwayFromZero);
			}

			if (size.x < 1 || size.y < 1 || size == currentSize)
				return image;

			scale = size / (double2)currentSize;
            Image<Argb32> dest = new(size.x, size.y);

			// Downscale with a modified Box algorithm that treats edge pixels as partial pixels for fractional coordinates
			...
			Parallel.For(0, size.y, y =>
			{
				...
				var rowY1 = image.DangerousGetPixelRowMemory(y1).Span;
				...
				var rowY2 = image.DangerousGetPixelRowMemory(y2).Span;
				...
				for (int x = 0; x < size.x; x++)
				{
					...
					destRow[x] = (pixel / (tlMult + trMult + blMult + brMult + innerWidth * (top + bottom) + innerHeight * (left + right) + innerWidth * innerHeight)).ToArgb32();
				}
			});
